...
def main():
    with PG_CONN.cursor() as cur:
        cur.execute("SELECT accession_number, cik, filing_date, xbrl_url FROM pipelines.sec13f_jobs WHERE status = 'FAILED'")
        jobs = cur.fetchall()
                ...
                for xml in xmls:
                    ...
                    if "edgarSubmission" in doc:
                        ...
                        if state["submission"] is None:
                            print(f"⚠️ Skipping informationTable due to missing submission data for {accession_number}")
                            continue  # or set a default period or handle as needed
